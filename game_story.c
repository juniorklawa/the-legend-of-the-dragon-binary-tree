/*
 *
 * Aqui ficam todas as frases de cada nó do jogo
 *
 * */

// ARVORE 2
static char *key1Description = "\nVocê acorda dentro de uma prisão, sem se lembrar muito como foi parar lá, enquanto você tenta se lembrar como foi parar preso um guarda aparece na porta e diz:\n"
                               "- De pé, prisioneiro. Alguém lá fora gosta muito de você e pagou sua fiança, pegue suas coisas e se retire da prisão.\n"
                               "\n"
                               "1. Pegar suas coisas e sair\n"
                               "2. Atacar o guarda";


static char *key2Description = "Ao sair da prisão você vai para a Taverna pedir uma bebida, quando um homem estranho aparece na sua frente.\n"
                               "\n"
                               "1. Perguntar ao Homem o que ele quer.\n"
                               "2. Ignorar o homem.";

static char *key3Description = "Por algum motivo (que nem você entende direito) você ataca o guarda o derruba no chão. \n"
                               "Os outros soldados logo percebem o ataque e partem pra cima de você, eles te imobilizam e dias depois você é condenado a forca.";


static char *key4Description = "Você olha para o homem e pergunta o que ele quer";

static char *key5Description = "Você ignora o homem e continua a beber";

// ARVORE 2

static char *key6Description = "O homem se aproxima da mesa, sem falar nada e deixa uma carta e sai.\n"
                               "\n"
                               "Após achar tudo isso muito estranho, você espera o homem sair do local e abre a carta:\n"
                               "A carta diz:\n"
                               "'Siga para o Sul, encontra a caverna secreta, mate o Dragão e recupere meu tesouro, você é o unico capaz de fazer isso!'\n"
                               "Você fecha a carta e:\n"
                               "\n"
                               "1. Vai até o dono da Taverna perguntar se ele sabem quem era o homem misterioso.\n"
                               "2. Sai com pressa ta taberna para chegar o mais rapido possivel no lugar que o mapa indica";

static char *key7Description = "Você pergunta para o dono da Taverna quem era aquele homem e ele te responde:\n"
                               "- Ele me pediu para não revelar sua identidade, mas me disse que você deveria escolher um desses dois equipamentos.\n"
                               "O Taberneiro te leva para a parte de trás do local e te mostra equipamentos de um guerreiro e equipamentos de um mago e um cavalo, o que você escolhe?\n"
                               "\n"
                               "1. Guerreiro\n"
                               "2. Mago";

static char *key8Description = "Você sai com pressa da taberna em direção ao sul, mas vocẽ não tem equipamento,  comida, e nem cavalo. \n"
                               "Após 4h de viagem, um grupo de Globins te ataca e você morre.";

static char *key9Description = "Você olha para a espada e se lembra da sua infância enquanto treinava com seu mestre, o quanto almejava ser alguém lendário, um herói, você escolhe o equipamento de aço e couro curtido.";

static char *key10Description = "Você olha para o manto e um cajado e lembra da sua adolescência quando passou debruçado em livros empoeirados aprendendo a arte da magia, foi um caminho solitário cercado por paredes de livros mas a certeza de ter se tornado alguém único. \n"
                                "O cajado encaixa perfeitamente em sua mão e você ja tinha sua escolha.";

static char *key11Description = "Você pega seu novo equipamento, guardas suas poções na sua bolsa, agradece ao dono da Taverna e se prepara pra sair.\n"
                                "\n"
                                "1. Perguntar ao Taverneiro sobre informações do Sul. \n"
                                "2. Sair despreocupadamente";

static char *key12Description = "Você pergunta sobre informações do sul, descobre que a região no período que você estava preso se tornou caótica com o surgimento de uma energia vindo de uma caverna, \n"
                                "até então ninguém havia explorado e descoberto o motivo, porem monstros habitavam todos os cantos ao Sul.\n"
                                "\n"
                                "Você sai pela guarnição tomando todo o cuidado, descobre rastros de um grupo de globins e ouros monstros, avista 4 globins e um hobglobin, além de um Globin mágico escondido entre as árvores de vigia. \n"
                                "Após isto você encontra um caminho que leva ao ninho dos globins e decide investigar.";

static char *key13Description = "Você sai da taverna, caminha até a guarnição que cuidava da rota ao Sul, se pergunta porque a defesa é tão forte em direção ao Sul.\n"
                                "Você passa pela guarnição tomando o mínimo de cuidado necessário. Você avista um grupo de globins se alimentando de algo.\n"
                                "\n"
                                "1. Apenas contornar e seguir o caminho, agora mais atento sabendo da existência de monstros na região.\n"
                                "2. Atacar o grupo enquanto eles estão despreocupados\n";

static char *key15Description = "Você contorna o grupo de monstros, nenhum deles nota sua presença, porem você repara que todos os passos dos globins levam a uma direção, \n"
                                "você decide rastrear o ninho para descobrir novas informações.";

static char *key16Description = "Você decide realizar um ataque surpresa ao grupo de globins, são 4 globins pequenos e 1 hobglobins.\n";




// ARVORE 4

static char *key17Description = "A caverna globin se abriu a sua frente, você acendeu uma pequena tocha e caminhou por varios minutos seguindo as paredes escuras, varias marcas de pés manchavam o chão. \n"
                                "Você encontra uma bifurcação, a direita uma caverna mas ampla se abre, com varias marcas de pés saindo dela, a sua esquerda é uma caverna menor, porem mais funda e escura.\n"
                                "\n"
                                "1. Direita\n"
                                "2. Esquerda";

static char *key18Description = "Você pergunta sobre informações do sul, descobre que a região no período que você estava preso se tornou caótica com o surgimento de uma energia vindo de uma caverna, \n"
                                "até então ninguém havia explorado e descoberto o motivo, porem monstros habitavam todos os cantos ao Sul.\n"
                                "\n"
                                "Após uma grande caminhada você encontra um salão com varias peças em ouro, contradizendo a natureza primitiva dos globins, \n"
                                "em um trono improvisado se sentava um globin grande com uma coroa, ele com sua natureza hostil te assusta e você decide aproveitar a chance enfrentar o Goblin Rei.\n";

static char *key19Description = "Você entra na bifurcação a direita, a entrada é mais ampla e sua tocha mal é capaz de iluminar o ambiente, \n"
                                "um grande salão se abre a sua frente, você levanta sua tocha para iluminar mais a fundo e revela varios pequenos globins assustados, \n"
                                "este salão era uma espécie de dormitório, varias camas improvisadas se espalhavam. \n"
                                "Você decide voltar antes de acabar emboscado em uma caverna sem saída, retornando você segue o caminho da esquerda.\n"
                                "\n"
                                "Após uma grande caminhada você encontra um salão com varias peças em ouro, contradizendo a natureza primitiva dos globins, \n"
                                "em um trono improvisado se sentava um globin grande com uma coroa, ele com sua natureza hostil te assusta e você decide aproveitar a chance enfrentar o Goblin Rei.\n"
                                "\n";


static char *key22Description = "Após derrotar o rei goblin, você encontra uns documentos em seu manto. \n"
                                "Você escuta varios barulhos vindo do corredor, sua saida principal havia sido bloqueada, atrás do trono uma porta pesada imponente atraia sua atenção\n"
                                "\n"
                                "1. Abrir a porta na esperança de achar uma saida\n"
                                "2. Enfrentar o grupo de goblins e abrir seu caminho ";

static char *key23Description = "Você abre a porta, ela desliza suavamente contradizendo seu enorme tamanho, uma espécie de corredor cobertos de livros se abre a sua frente. \n"
                                "Uma mesa com um livro imponente no canto atrai atenção, você pega o livro, agora resta uma duvida:\n"
                                "\n"
                                "1. Seguir o corredor correndo o risco de ficar encurralado em um espaço estreito\n"
                                "\n"
                                "2. Voltar e enfrentar o grupo no salão principal ";

static char *key24Description = "Você percebe que o grupo está mais distante, você observa o salão e começa a pensar em uma estrategia\n"
                                "\n"
                                "1. Enfrentar o grupo de frente confiante na sua habilidade\n"
                                "2. Se esconder atrás do trono e esperar por uma oportunidade\n";

static char *key25Description = "Você corre pelo corredor, ele parecia subir gradualmente, incontáveis livros decoram o lugar, você se perguntaria o porque de goblins guardarem livros, mas após ver a inteligencia e como o rei goblin se portava, aqueles livros pareciam ser todos dele. \n"
                                "Uma porta aparece a sua frente, você a abre devagar e uma espécie de sala se abre, era uma cabana em meio a uma floresta, você estava salvo.";

static char *key26Description = "Você retorna, um grupo de muitos goblins aparece a sua frente, eles parecem desorientados devido a morte de seu soberano, você resolve atacar eles, afinal era a sua unica escapatoria.\n"
                                "\n";

static char *key27Description = "Você decide enfrentar o grupo de frente, derrota dois goblins no primeiro ataque surpresa, agora um grupo de varios goblins te encaravam e estavam atentos.\n"
                                "\n";

static char *key28Description = "Você se esconde atrás do trono, o grupo de goblins entra, se assusta com a situação, não se aproximam do trono por medo e respeito, pegam o corpo do seu rei e levam para a câmara interna, o caminho se abre e você consegue fugir apenas matando dois goblins furtivamente, você estava na floresta novamente.";

static char *key29Description = "\nApós bravamente derrotar os Goblins, você olha para os lados e vê apenas arvores, o documento que você achou no corpo do Rei goblin parece ter um mapa e informações sobre um inimigo \"supremo\", os monstros o chamam de A Grande chama, parece que sua caverna fica embaixo de uma cachoeira no sul, dizem que seu corpo é integralmente de chamas e ele mora ali para evitar botar fogo na floresta com seu calor.\n"
                                "Você caminha em direção ao sul e encontra um baú trancado.\n"
                                "\n"
                                "1. Abrir\n"
                                "2. Ignorar";

static char *key30Description = "Você abre o baú, na verdade ele era um mimico, uma espécie de monstro em formato de bau, são criaturas dificeis de se encontrar\n";

static char *key31Description = "Você ignora o bau e segue seu mapa em direção ao sul, é relativamente simples o trajeto, basta seguir o rio acima, e quando achassem a cachoeira achariam a caverna.";

static char *key33Description = "Você encontra a cachoeira, um grande lago se abre a sua frente com uma cachoeira inimaginavelmente grande, eram varias quedas que alagavam uma imensa piscina, você precisa pensar em como enfrentar o temível monstro..\n"
                                "\n"
                                "1. Você entra na caverna cautelosamente, largou a ideia de molhar suas roupas para manter sua velocidade alta e mobilidade para enfrentar ele.\n"
                                "2. Você decide mergulhar no lago para molhar suas roupas e equipamentos para tornar elas mais resistente a fogo\n";

static char *key34Description = "Você entra na caverna e encontra uma pilha de ouro com um dragão de fogo em cima, seu corpo era inteiramente de fogo e tu se pergunta como o ouro não derretia com aquele calor.\n"
                                "Dragões são criaturas gananciosas e estocam normalmente bens materiais, mas a pilha ali era imensa. Uma grande joia pairava em um pedestal do outro lado do salão, aquilo te chamou atenção\n"
                                "\n"
                                "1. Atacar diretamente o dragão enquanto ele dorme, tentando alcançar seu coração\n"
                                "2. Ir até a joia de ouro e pegar ela";

static char *key35Description = "Você mergulha no lago até suas roupas e equipamentos ficarem encharcados, você sente seu equipamento te pesar com o peso da agua.\n"
                                "Você entra na caverna e encontra uma pilha de ouro com um dragão de fogo em cima, seu corpo era inteiramente de fogo e tu se pergunta como o ouro não derretia com aquele calor.\n"
                                "Dragões são criaturas gananciosas e estocam normalmente bens materiais, mas a pilha ali era imensa.\n"
                                "Uma grande joia pairava em um pedestal do outro lado do salão, aquilo te chamou atenção\n"
                                "\n"
                                "1. Ir até a joia de ouro e pegar ela\n"
                                "2. Atacar diretamente o dragão enquanto ele dorme, tentando alcançar seu coração";

static char *key36Description = "Você ataca o dragão, seu ataque é efetivo e o dragão acorda assustado, você tão assustado quanto ele, acaba por tropeçar em varias moedas, o dragão solta uma baforada de fogo em você,\n"
                                "Seu equipamento acaba por não resistir e você acaba sendo queimado e não resiste. Você morreu.";

static char *key37Description = "Você vai até a joia, observa ela com total silencio, você toca nela.\n"
                                "Quando você toca na joia o dragão acorda furioso por alguem ter tocado em seu tesouro, você em um ato de susto, resolve correr em direção a saída.\n"
                                "Você consegue correr rapidamente e aos tropeços vai saindo, o dragão no ato furioso e cego resolve te seguir, você sai pela abertura da caverna e ele esquece a sua fraqueza.\n"
                                "Ele é atingido por uma imensa quantidade de agua que acaba por causar um imenso dano nele, o fogo do seu corpo começa a se apagar e ele cai na piscina morrendo sem conseguir sair de lá.";

static char *key38Description = "Você vai até a joia, observa ela com total silencio, você toca nela.\n"
                                "Quando você toca na joia o dragão acorda furioso por alguem ter tocado em seu tesouro, você em um ato de susto, resolve correr em direção a saída, seu equipamento molhado te atrapalha, você acaba por tropeçar e recebe uma patada do dragão, seu equipamento não resiste e você morre.";

static char *key39Description = "Você ataca o dragão, seu ataque é efetivo e o dragão acorda assustado, você tão assustado quanto ele, acaba por tropeçar em varias moedas, o dragão solta uma baforada de fogo em você, você acha que ia morrer, porem seu equipamento encharcado acaba por lhe prover uma resistência. Você se levanta e se prepara pra luta.\n";

static char *key40Description = "Você volta a caverna, coleta o que você consegue de ouro e a joia.\n"
                                "Você se sente tentado com todo aquele ouro e se lembra da carta \"Devolva meu tesouro\"\n"
                                "\n"
                                "1. Ficar com todo o ouro pra si\n"
                                "2. Devolver o ouro ao dono da carta";

static char *key41Description = "Você decide ficar com todo o ouro, suas mãos estão manchadas de sangue e você decide viver o resto da vida confortavelmente após tamanho sacrifício que foi terminar esta missão.\n"
                                "Você retorna a cidade, espalha a informação que não conseguiu concluir a missão e acaba por viver confortavelmente na taverna mais cara com recursos praticamente infinitos.";

static char *key42Description = "Você decide devolver o ouro, você sela a caverna e retorna a cidade para informar a localização da pilha de ouro.\n"
                                "Você encontra o taverneiro e ele se mostra ser o emissor da carta, Você monta um mapa com a localização exata e avisa que o caminho está seguro com a ausência de monstros após a morte do dragão.\n"
                                "Ele te permite ficar com o ouro que você coletou e com a joia, e promete te recompensar apropriadamente quando reaver o que é seu";









